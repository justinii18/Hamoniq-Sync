...
internal final class ProgressCallbackManager {
    ...
    lazy var cCallback: ProgressCallback = { [weak self] (progress, operation, userData) in
        guard let self = self else { return 0 }
        
        // Throttle progress updates to max 10 Hz (every 100ms)
        let now = Date()
        if now.timeIntervalSince(self.lastProgressTime) < 0.1 && progress < 1.0 {
            return 0 // Continue processing
        }
        self.lastProgressTime = now
        
        let operationString = operation != nil ? String(cString: operation!) : self.currentStage.displayName
        
        // Estimate time remaining
        let elapsed = now.timeIntervalSince(self.startTime)
        let estimatedTotal = elapsed / max(progress, 0.01) // Avoid division by zero
        let remaining = max(0, estimatedTotal - elapsed)
        
        let syncProgress = SyncProgress(
            stage: self.currentStage,
            percentage: progress * 100.0,
            estimatedTimeRemaining: remaining > 0.1 ? remaining : nil,
            currentOperation: operationString,
            processedSamples: Int64(progress * Double(self.totalSamples)),
...
internal extension SyncConfiguration {
